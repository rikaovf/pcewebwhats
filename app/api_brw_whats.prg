...
function Api_Brw_whats( oDom )
	...
	do case
		...
		case oDom:GetProc() == 'vinculacontato'
			vinculaContato( oDom )	
		case oDom:GetProc() == 'vinculachavecli'
			vinculaChaveCli( oDom )
		case oDom:GetProc() == 'deletaconversa'
			deletaConversa( oDom )
...
procedure vinculaChaveCli(oDom)

	local hErro := {=>}
	local nId := oDom:Get('idRetag')
	local cNomeCli := oDom:Get('nomeCli')
	local cChaveCli := oDom:Get('chaveCli')
...
procedure deletaConversa( oDom )
	...
	if ace398->( dbSeek(str(nId, 11)) )
		...
		ace397->( setScope(0, str(ace398->id_retag,11) ) )
		ace397->( setScope(1, str(ace398->id_retag,11) ) )
		ace397->( DbGoTop() )
		...
		GravaLogWhatsapp ("Chat Excluido: " + ace398->n_whatsapp + " - " + DTOC(DATE()) + "-" + TIME() + " por: " + ALLTRIM( CODTODES( 'F'+hData['func'], 'ACE004.001','ICE004','TAB_DES')) )

		HB_HSet( hResult, 'erro', {'Chat apagado com sucesso', 'Sucesso'} )
	else
		HB_HSet( hResult, 'erro', {'Erro ao apagar chat da base de dados da concentra', 'Erro'} )
	endif
	
	oDom:SetJs('errorMsgApi', hResult)
